...
function onOpen() {
  ...
  if (isAdmin) {
    SpreadsheetApp.getUi().createMenu('🛠️ 開團管理')
      ...
      .addSeparator()
      .addItem('── 商品管理 ──', 'noopMenuLabel_')
      .addItem('🔧 初次設定（建立分頁）', 'pmSetupSheet')
      .addItem('📥 匯入指定結單日商品', 'pmImportByEndDate')
      .addItem('✅ 同步選定商品到 products', 'pmSyncSelected')
      .addItem('✅ 同步全部未同步到 products', 'pmSyncAllUnsynced')
      .addItem('🔍 搜尋商品並補入', 'pmOpenSearchDialog')
      .addSeparator()
