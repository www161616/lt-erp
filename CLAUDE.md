...
- 不確定某個欄位是否存在或型別是什麼
- 發現 UI 流程在手機上可能有問題
- **搬家型改動（資料來源遷移，如 shared_kv → DB、localStorage → DB）**
...
### 搬家型改動強制規則（資料來源遷移）
**歷史教訓（04/15 事故）**：銷貨單從 shared_kv 搬到 DB 時，只改了讀取端沒改寫入端，導致店家退貨回報失效（RLS 擋 PATCH）、admin 看不到退貨通知、銷貨單明細空白。修了一整天。

**任何把資料來源從 A 換到 B 的改動，必須嚴格執行以下步驟：**

1. **列出所有讀取點**：grep 舊資料來源名稱，找出所有讀取的地方
2. **列出所有寫入點**：grep 所有 PATCH/POST/cloudSave/localStorage.setItem 到舊來源的地方
3. **確認 RLS 權限**：新資料來源的每張表，5 個角色（admin/assistant/accountant/staff/store）分別能 SELECT/INSERT/UPDATE/DELETE 嗎？
4. **跨角色測試**：改完後必須用 admin 帳號 + store 帳號各跑一輪完整流程
5. **測試項目必須包含**：
   - 列表顯示 ✓
   - 點開明細 ✓
   - 新增/編輯/刪除 ✓
...
- **不允許只改讀取端不改寫入端就 push（搬家型改動最常犯的錯）**
- **不允許跳過跨角色測試就宣稱「已完成」**
