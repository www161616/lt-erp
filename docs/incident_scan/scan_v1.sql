...
--   Scan #2 紅燈 - order_date 與單號日期不一致（BUG-001/015 類）
...
-- 涵蓋：SO、RT 前綴
-- 已排除：舊格式 SO00001 / SO-20260323-134 等無法從 ID 取日期的
-- 誤報候選：
--   - 深夜跨 UTC/Taipei 邊界可能誤報（下一版加時區容忍）
--   - 合法的人工改日期（例：4/21 誤建 4/20 的單，手動修正）
...
where
  ...
  and sales_orders.order_date
      is distinct from to_date(substring(sales_orders.id from 3 for 8), 'YYYYMMDD')
order by
  sales_orders.order_date desc,
  sales_orders.id desc;
